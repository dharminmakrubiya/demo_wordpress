...
class Mappress_Map extends Mappress_Obj {
	...
	static function ajax_duplicate() {
		...
		$map = self::duplicate($mapid, $otype, $oid);
		if ($map)
	...
	*/
	static function deleted_post($postid) {
		...
		foreach($mapids as $mapid)
			$result = $result && self::mutate($mapid, array('status' => 'trashed', 'oid' => 0));
		return $result;
	}
	...
	*/
	...
	function display_iframe() {
		...
		$html = "<div class='mapp-iframe-layout'>"
			...
			. "<iframe class='mapp-iframe' src='$url' loading='lazy'></iframe>"
			. "</div>"
			. "</div>";
	...
	static function duplicate($mapid, $otype, $oid) {
		$map = self::get($mapid);
		if (!$map)
			return null;
