...
if( !class_exists( 'VisionWP_Main' ) ) {
	class VisionWP_Main{
		...
		public function customize_register( $wp_customize ) {
			...
			self::kirki_required_msg( $wp_customize );
		}

		/**
		 * Notice on customizer
		 * @since 1.0.0
		 * @package VisionWP WordPress Theme
		 */
		public static function kirki_required_msg( $wp_customize ){
			if( !class_exists( 'Kirki' ) ){				

				$wp_customize->add_section( 'kirki_required' , array(
				  'title' => esc_html__( 'To enable features, install Kirki Customizer Framework Plugin', 'visionwp' ),
				  'priority' => 0,
				));

				$wp_customize->add_setting( 'kirki_recommended', array(
				  'default' => '',
				  'sanitize_callback' => 'text',
				));

				$wp_customize->add_control( 'kirki_recommended', array(
				  'label' => esc_html__( 'Hidden Control', 'visionwp' ),
				  'type' => 'text',
				  'section' => 'kirki_required',
				));
			}
		}
