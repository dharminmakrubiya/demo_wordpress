...
if( !class_exists( 'VisionWP_Welcome_Notice' ) ){	
	class VisionWP_Welcome_Notice{
		...
		public static function notice_init( $theme_name = false ){
			...
			# welcome Note
			if( 'dismiss_notice' !== get_transient( 'visionwp_welcome_notice' ) ){
		...
		public static function rt_welcome_ajax_action_close(){
			$response = array(
				'data' => array(
					'message' => esc_html__( 'Invalid Access', 'visionwp' )
				),
				'status' => 400
			);

			if( wp_verify_nonce( $_POST['nonce'], 'rt-welcome-nonce' ) ){
				set_transient( 'visionwp_welcome_notice', 'dismiss_notice', 3*MONTH_IN_SECONDS );

				$response[ 'data' ][ 'message' ] = esc_html__( 'Notice dismissed.', 'visionwp' );
				$response[ 'status' ] = 200;
			}
		...
		public static function rt_welcome_ajax_action(){
			...
			if( wp_verify_nonce( $_POST['nonce'], 'rt-welcome-nonce' ) ){
				...
				$response[ 'data' ][ 'message' ] = esc_html__( 'Plugin Installed and Activated Successfully', 'visionwp' );
				$response[ 'status' ] = 200;
			}
		...
		public static function display_welcome_notice(){
			set_query_var( 'visionwp_welcome_notice_theme', self::$theme_detail );
			get_template_part( 'class/welcome-notice/templates/content', 'welcome' );
		}
		...
		public static function module_admin_scripts(){
			wp_enqueue_style( 'welcome-style', get_theme_file_uri( '/class/welcome-notice/assets/css/welcome-notice.css' ) );
			wp_enqueue_script( 'welcome-script', get_theme_file_uri( '/class/welcome-notice/assets/js/welcome-notice.js' ), array( 'jquery' ) );

			$data = array(
				'admin_url'	=> admin_url( 'admin-ajax.php' ),
				'nonce' => wp_create_nonce('rt-welcome-nonce'),
				'confirm_msg' => esc_html__( 'Are you sure?', 'visionwp' ),
				'install_msg' => esc_html__( 'Installing...', 'visionwp' ),
				'active_msg' => esc_html__( 'Activating...', 'visionwp' )
			);
